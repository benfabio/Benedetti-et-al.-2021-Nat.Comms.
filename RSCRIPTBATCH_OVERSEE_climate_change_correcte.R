...
### Define niche modelling function and projection for mclapply
SpeciesNicheModelling <- function(sp = species) {
							...
							### 03/05/19: watch out for NAs in env predictors like Chl or PAR that might remove presences & absences
							...
							if(n >= 75) {
								...
								setwd(paste(WD,"/","niche.modelling_future/eval_scores_",p,"/", sep = ""))
...
donespp <- str_replace_all(dir(), ".Rdata", "")
donespp <- str_replace_all(donespp, "eval_scores_", "")
donespp <- gsub("\\.", "_", donespp) # But also need to add brackets for some annoying species names
# OK Now remove the 'donespp' from 'species'
species2 <- species[!(species %in% donespp)]

# Plus those that present NAs in the summary tables of eval_scores
species2 <- c("Calanus_helgolandicus","Centropages_typicus",species2)

setwd(paste(WD,"/niche.modelling_future/",p,"/", sep = ""))
second.wd <- getwd()

setwd(WD)
	
mclapply(X = species2, SpeciesNicheModelling, mc.cores = 25)
